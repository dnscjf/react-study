...
: React js 버전으로 진행. [CSR] ( 나중에 typescript js 버전으로 진행 )
: React ts 버전으로 진행.
: Next ts 버전으로 진행. [SSR]
...
: npm install 라이브러리 명을 이용하여 다운로드 받고 활용
: 만약 프로젝트에서 라이브러리 등의 오류가 발생 시

- node_modules 폴더 삭제
- package-lock.json 파일 삭제
- 이후 `npm i`를 작성 후 실행(터미널)
  : 깃허브 push 대상에서 제외하기 위해 .gitignore에 자동 입력

## public 폴더 (압축 안됨 = Webpack에서 제외됨)

: index.html - 최초 웹서비스 실행시 실행되는 파일명(파일명 변경 불가) - lang = "en" → "ko"로 수정 - favicon 아이콘 수정 - SEO 관련 내용은 별도 추가 - apple-touch-icon 수정 - title 내용 수정 - manifest.json 은 추후 내용 수정필요
(https://web.dev/articles/add-manifest?hl=ko) - id="root"는 리액트 미리보기 및 결과물이 배치되는 장소
...
- node-modules에서 사용된 라이브러리들의 버전 보관
